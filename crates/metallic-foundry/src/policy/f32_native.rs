...
use crate::{
    F32, Foundry, compound::Layout, spec::{FastBindings, ResolvedSymbols}, tensor::{Tensor, TensorInit}, types::TensorArg
};

#[derive(Debug, Clone, Default, MetalPolicy)]
#[policy(
    header = "policies/policy_f32.metal",
    struct_name = "PolicyF32",
    short_name = "f32_native",
    element_size = 4,
    block_size = 1,
    vector_load_size = 16,
    unroll_factor = 2,
...
pub struct PolicyF32Native;

impl LoaderStage for PolicyF32Native {
    ...
    fn bind(&self, fast_bindings: &FastBindings, resolved: &ResolvedSymbols) -> smallvec::SmallVec<[TensorArg; 4]> {
        ...
        let tensor = fast_bindings.get(resolved.weights).expect("F32 native weight bound");
        smallvec![tensor.clone()]
    }

    fn quantization_type(&self) -> std::sync::Arc<dyn super::MetalPolicyRuntime> {
        std::sync::Arc::new(PolicyF32Native)
    }
}

impl MetalPolicyRuntime for PolicyF32Native {
    ...
    fn load_weights(
        ...
        let data_f32: Vec<f32> = bytemuck::try_cast_slice(data_bytes)
            .map_err(|e| anyhow::anyhow!("Invalid F32 bytes for {}: {}", source_tensor_name, e))?
            .to_vec();
        ...
        if let Layout::Canonical { expected_k, expected_n } = layout {
            ...
            let mut canonical_data = vec![0.0f32; canonical_len];
                ...
                for block in 0..blocks_per_k {
                    ...
                    if remaining >= WEIGHTS_PER_BLOCK {
                        for i in 0..WEIGHTS_PER_BLOCK {
                            ...
                            canonical_data[dst_base + i] = data_f32[src_idx];
                        }
                    } else if remaining > 0 {
            ...
            let tensor = Tensor::<F32>::new(foundry, vec![canonical_len], TensorInit::CopyFrom(&canonical_data[..]))
            ...
            let data_to_upload = if is_kn {
                ...
                for row_k in 0..k {
                    for col_n in 0..n {
                        transposed[col_n * k + row_k] = data_f32[row_k * n + col_n];
                    }
                }
                transposed
            } else {
            ...
            let tensor = Tensor::<F32>::new(foundry, vec![n, k], TensorInit::CopyFrom(&data_to_upload[..]))
                .map_err(|e| anyhow::anyhow!("Metal error: {:?}", e))?;
            Ok(vec![(logical_name.to_string(), TensorArg::from_tensor(&tensor))])
        } else {
            let tensor = Tensor::<F32>::new(foundry, dims.clone(), TensorInit::CopyFrom(&data_f32[..]))
                .map_err(|e| anyhow::anyhow!("Metal error: {:?}", e))?;
            Ok(vec![(logical_name.to_string(), TensorArg::from_tensor(&tensor))])
        }
    }
