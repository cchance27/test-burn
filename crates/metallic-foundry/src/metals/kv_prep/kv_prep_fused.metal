...
/// - Rearrange + RoPE for K and write into expanded cache [n_heads, max_seq_len, head_dim]
/// - Rearrange for V and write into expanded cache [n_heads, max_seq_len, head_dim]
///
...
kernel void kv_prep_fused_kernel_f16(
    ...
    // === K/V: Rearrange (+RoPE for K) and write to repeated cache ===
    ...
    // Cache is [n_heads, max_seq_len, head_dim]
    ...
    // Repeat KV head across group_size query heads.
    const uint base_head = kv_h * group_size;
    const uint max_heads = params->n_heads;
    for (uint r = 0; r < group_size; ++r) {
        const uint out_head = base_head + r;
        if (out_head >= max_heads) break;
        const uint cache_idx = out_head * cache_head_stride + cache_row_base;
