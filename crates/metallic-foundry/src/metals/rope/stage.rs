...
#[stage(
    ...
    emit = r#"
    threadgroup RopeSharedVec4T q_shared[ROPE_MAX_HEAD_DIM_VEC];
    run_rope_decode_stage(q_shared, q_ptr, cos, sin, params_rope, lid.x);
    const threadgroup RopeSharedVec4T* q_ptr_roped = q_shared;
"#,
    out_var = "q_ptr_roped"
)]
pub struct RopeStage {
    #[arg(buffer = 12, metal_type = "const device TensorStorageT*")]
    pub cos: TensorArg,
    #[arg(buffer = 13, metal_type = "const device TensorStorageT*")]
    pub sin: TensorArg,
