...
pub struct SdpaOnlineStage {
    pub sdpa_params: SdpaParams,
}

impl Stage for SdpaOnlineStage {
    fn includes(&self) -> Vec<&'static str> {
        vec!["simd.metal", "sdpa/sdpa_online.metal"]
    }
...
/// Standalone SDPA Stage - loads Q directly from buffer (no RoPE fusion).
///
/// Use this stage when SDPA is not fused with RoPE and Q needs to be loaded
/// from the global buffer instead of shared memory.
#[derive(KernelArgs, Clone, Debug)]
pub struct SdpaStandaloneStage {
    pub sdpa_params: SdpaParams,
}

impl Stage for SdpaStandaloneStage {
    fn includes(&self) -> Vec<&'static str> {
        vec!["simd.metal", "sdpa/sdpa_online.metal"]
    }
    ...
    fn emit(&self, _input_var: &str) -> (String, String) {
        (
            "output_ptr".to_string(),
            r#"
    // SdpaStandaloneStage - loads Q directly from buffer
    #define MAX_REDUCE_DIM_VEC 64 // 256/4
    threadgroup float reduce_shared[MAX_REDUCE_DIM_VEC];
