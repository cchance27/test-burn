...
use crate::{
    F16, Foundry, MetalError, compound::{CompiledCompoundKernel, CompoundKernel, Layout, stages::WarpLayoutStage}, fusion::MetalPolicy, metals::rmsnorm::stages::RmsNormComputeStage, spec::{CompiledStep, FastBindings, Ref, ResolvedSymbols, Step, SymbolTable, TensorBindings}, storage::Pooled, tensor::{Tensor as FoundryTensor, TensorInit}, types::{DispatchConfig, MetalResourceOptions, TensorArg}
};
...
impl CompiledStep for CompiledFusedSwigluStep {
    fn execute(
        ...
        if w_gate.dtype != w_up.dtype {
            ...
            let mut run_projection = |policy: std::sync::Arc<dyn crate::policy::MetalPolicyRuntime>,
                                      resolved: &ResolvedSymbols,
                                      dst: TensorArg|
             -> Result<(), MetalError> {
                let loader = policy.loader_stage();
                let bound = loader.bind(_fast_bindings, resolved);
            ...
            run_projection(policy_gate.clone(), &self.wg_resolved, gate_out.clone())?;
            run_projection(policy_up.clone(), &self.wu_resolved, up_out.clone())?;

            let zero_bias = vec![f16::from_f32(0.0); n_dim as usize];
            let zero_bias_tensor = FoundryTensor::<F16, Pooled>::new(foundry, vec![n_dim as usize], TensorInit::CopyFrom(&zero_bias))
                .map_err(|e| MetalError::OperationFailed(format!("mixed-swiglu zero bias alloc failed: {e}")))?;
...
impl CompiledStep for CompiledFusedFfnSwiGluRmsNormStep {
    fn execute(
        ...
        if w_gate.dtype != w_up.dtype {
            ...
            run_projection(policy_gate.clone(), &self.w_gate_resolved, gate_out.clone())?;
            run_projection(policy_up.clone(), &self.w_up_resolved, up_out.clone())?;

            let zero_bias = vec![f16::from_f32(0.0); n_dim as usize];
            let zero_bias_tensor = FoundryTensor::<F16, Pooled>::new(foundry, vec![n_dim as usize], TensorInit::CopyFrom(&zero_bias))
                .map_err(|e| MetalError::OperationFailed(format!("mixed-ffn zero bias alloc failed: {e}")))?;
            let gate_bias_tensor = if has_b_gate != 0 {
                b_gate.clone()
            } else {
                TensorArg::from_tensor(&zero_bias_tensor)
