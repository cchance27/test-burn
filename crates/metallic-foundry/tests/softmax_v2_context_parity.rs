...
use metallic_context::{
    Context, F16Element, kernels::softmax_kernel::SoftmaxKernelOp, tensor::{Tensor as LegacyTensor, TensorInit as LegacyTensorInit, TensorStorage as LegacyStorage}
};
use metallic_foundry::{
    Foundry, MetalError, metals::softmax::SoftmaxV2Step, spec::{DynamicValue, FastBindings, Ref, Step, SymbolTable, TensorBindings}, storage::Pooled, tensor::{Tensor as FoundryTensor, TensorInit, dtypes::F16 as F16Dtype}, types::TensorArg
};
...
fn run_softmax_parity_test(rows: usize, cols: usize, causal: bool) -> Result<(), MetalError> {
    ...
    let res_v2: Vec<f16> = output_v2.to_vec(&foundry);

    // --- Context Softmax ---
    let mut ctx = Context::<F16Element>::new().unwrap();

    let input_ctx = LegacyTensor::<F16Element>::new(
