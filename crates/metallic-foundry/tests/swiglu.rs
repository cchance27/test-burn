...
fn cpu_swiglu_fused(gate: &[f16], up: &[f16], gate_bias: &[f16], up_bias: &[f16], batch: usize, hidden_dim: usize) -> Vec<f16> {
    ...
    for (gid, val) in output.iter_mut().enumerate().take(total) {
...
fn run_parity_test(cfg: TestConfig) {
    ...
    // =========================================================================
    let gate_foundry =
        FoundryTensor::<FoundryF16, Pooled>::new(&mut foundry, vec![total_elements], TensorInit::CopyFrom(&gate_data)).unwrap();
    let up_foundry = FoundryTensor::<FoundryF16, Pooled>::new(&mut foundry, vec![total_elements], TensorInit::CopyFrom(&up_data)).unwrap();
    let gate_bias_foundry =
        FoundryTensor::<FoundryF16, Pooled>::new(&mut foundry, vec![cfg.hidden_dim], TensorInit::CopyFrom(&gate_bias_data)).unwrap();
    let up_bias_foundry =
        FoundryTensor::<FoundryF16, Pooled>::new(&mut foundry, vec![cfg.hidden_dim], TensorInit::CopyFrom(&up_bias_data)).unwrap();

    let vector_width = if cfg.hidden_dim.is_multiple_of(4) { 4 } else { 1 };
