...
fn dense_nt_to_kn(tensor_nt: &Tensor<metallic_context::tensor::F16>, n: usize, k: usize) -> Vec<f32> {
...
fn assert_close_slice(a: &Tensor<metallic_context::tensor::F16>, b: &Tensor<metallic_context::tensor::F16>, name: &str) {
...
fn q8_gemv_parity_qwen25_shapes() -> Result<(), MetalError> {
    for s in SHAPES {
        ...
        let blocks_per_k = s.k.div_ceil(Q8_0_WEIGHTS_PER_BLOCK);
        ...
        for (blk, block) in y_ref_blocks_alt.iter_mut().enumerate().take(blocks_per_k) {
            let base_k = blk * Q8_0_WEIGHTS_PER_BLOCK;
            for col in 0..s.n {
