...
use metallic_foundry::{
    Foundry, compound::stages::Layout, metals::gemv::{GemvStrategy, GemvV2Args, get_gemv_v2_kernel_f16, get_gemv_v2_kernel_q8, warp_dispatch_config}, storage::Pooled, tensor::{F16, Tensor as FoundryTensor, TensorInit}, types::TensorArg
};
use rand::{Rng, rng};
...
fn run_cpu_gemv_f16(k: usize, n: usize, layout: Layout, weights: &[f16], input: &[f16], bias: Option<&[f16]>, alpha: f32) -> Vec<f16> {
    ...
    for row in 0..n {
        let mut acc = 0.0f32;
        for (ki, &x_f16) in input.iter().enumerate().take(k) {
            let x = x_f16.to_f32();
