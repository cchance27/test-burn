...
mod tests {
    ...
    fn pack_q8_blocks(rows: usize, cols: usize, weights: &[i8]) -> Vec<u8> {
        ...
        for row in 0..rows {
            for block_idx in 0..blocks_per_row {
                ...
                let mut qs = [0i8; 32];
                for (i, val) in qs.iter_mut().enumerate() {
                    let col = start_col + i;
                    *val = if col < cols { weights[row * cols + col] } else { 0 };
                }
    ...
    fn assert_proxy_bias_matches_fp16(
        ...
        let x_tensor = Tensor::<metallic_context::tensor::F16>::from_f32_slice(vec![1, input_dim], TensorStorage::Pooled(ctx), x_f32)?;
        let bias = Tensor::<metallic_context::tensor::F16>::from_f32_slice(vec![output_dim], TensorStorage::Pooled(ctx), bias_f32)?;

        let q8_y = ctx.call::<MatmulGemvOp>(
            (&x_tensor.clone(), TensorType::Quant(QuantizedTensor::Q8_0(&q8)), false, None),
            None,
        )?;
        ...
        let weight_tensor =
            Tensor::<metallic_context::tensor::F16>::from_f32_slice(vec![input_dim, output_dim], TensorStorage::Pooled(ctx), &weights_f32)?;
        let fp16_y = ctx.call::<MatmulGemvOp>((&x_tensor, TensorType::Dense(&weight_tensor), false, None), None)?;
    ...
    fn q8_proxy_smoke() -> Result<(), MetalError> {
        ...
        let x = Tensor::<metallic_context::tensor::F16>::from_f32_slice(vec![1, k], TensorStorage::Pooled(&mut ctx), &x_f32)?;

        let q8_result = ctx.call::<MatmulGemvOp>((&x, TensorType::Quant(QuantizedTensor::Q8_0(&q8)), false, None), None)?;
    ...
    fn q8_proxy_bias_matches_fp16_matmul_nk_layout() -> Result<(), MetalError> {
        ...
        let x = Tensor::<metallic_context::tensor::F16>::from_f32_slice(vec![1, input_dim], TensorStorage::Pooled(&mut ctx), &x_f32)?;
        let bias = Tensor::<metallic_context::tensor::F16>::from_f32_slice(vec![output_dim], TensorStorage::Pooled(&mut ctx), &bias_f32)?;

        // Proxy result using NK layout should match FP16 GEMV with A = transpose(weights_nk) to [K,N]
        let q8_y = ctx.call::<MatmulGemvOp>((&x.clone(), TensorType::Quant(QuantizedTensor::Q8_0(&q8)), false, None), None)?;
        let q8_y = ctx.call::<BroadcastElemwiseAddInplaceOp>((q8_y, bias.clone()), None)?;
        ...
        let a_fp16 = Tensor::<metallic_context::tensor::F16>::from_f32_slice(
            vec![input_dim, output_dim],
            TensorStorage::Pooled(&mut ctx),
            &weights_kn,
        )?;
        let fp16_y = ctx.call::<MatmulGemvOp>((&x, TensorType::Dense(&a_fp16), false, None), None)?;
        let fp16_y = ctx.call::<BroadcastElemwiseAddInplaceOp>((fp16_y, bias), None)?;
