...
impl<T: TensorElement> Operation for FusedQ2Op<T> {
    ...
    fn bind_kernel_args(&self, encoder: &Retained<ProtocolObject<dyn MTLComputeCommandEncoder>>) {
        ...
        let fallback = &self.x;
        let (b0_buf, b0_off) = if let Some(bias) = &self.bias0 {
            (&bias.buf, bias.offset)
        } else {
            (&fallback.buf, fallback.offset)
        };
        let (b1_buf, b1_off) = if let Some(bias) = &self.bias1 {
            (&bias.buf, bias.offset)
        } else {
            (&fallback.buf, fallback.offset)
        };
...
impl DefaultKernelInvocable for MatmulGemvQ2FusedOp {
    ...
    fn new<'a, T: TensorElement>(
        ...
        let w1 = canonize(w1)?;
        let (n0, n1);
        if w0.logical_dims.len() != 2 || w1.logical_dims.len() != 2 {
            return Err(MetalError::InvalidShape("Quant tensors must be 2D".into()));
        }
        if w0.logical_dims[0] != k || w1.logical_dims[0] != k {
            return Err(MetalError::InvalidShape("K mismatch for fused2".into()));
        }
        ...
        let params = Q2Params {
            ...
            n0: n0 as u32,
            n1: n1 as u32,
            blocks_per_k: w0.blocks_per_k as u32,
            weights_per_block: w0.weights_per_block as u32,
        ...
        let mut label = if crate::profiling_state::get_profiling_state() {
            ...
            GpuProfilerLabel::fallback("gemv_q8_fused2")
        };
        ...
        let pipeline = pipeline.ok_or_else(|| MetalError::PipelineCreationFailed("MatmulGemvQ2FusedOp".to_string()))?;
        let op = FusedQ2Op {
            pipeline,
            x: x.clone(),
            w0,
