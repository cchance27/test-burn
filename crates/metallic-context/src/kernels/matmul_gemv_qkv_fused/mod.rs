...
pub struct MatmulGemvQkvFusedRmsnormOp;
...
struct FusedQkvRmsnormOp<T: TensorElement> {
    pipeline: Retained<ProtocolObject<dyn MTLComputePipelineState>>,
    x: Tensor<T>,
    gamma: Tensor<T>,
    wq: CanonicalQuantTensor,
...
impl<T: TensorElement> Operation for FusedQkvRmsnormOp<T> {
    fn encode(&self, command_buffer: &CommandBuffer, _cache: &mut ResourceCache) -> Result<(), MetalError> {
        ComputeKernelEncoder::new(command_buffer, &self.profiler_label)?
            .pipeline(&self.pipeline)
            .bind_kernel(self)
    ...
    fn bind_kernel_args(&self, encoder: &Retained<ProtocolObject<dyn MTLComputeCommandEncoder>>) {
        ...
        set_buffer(encoder, 13, bias_v_buf, bias_v_offset);
        set_buffer(encoder, 14, &self.gamma.buf, self.gamma.offset);
    }
}
...
impl DefaultKernelInvocable for MatmulGemvQkvFusedOp {
    ...
    fn new<'a, T: TensorElement>(
        ...
        let bias_q = bias_q_opt.cloned();
        let bias_k = bias_k_opt.cloned();
        let bias_v = bias_v_opt.cloned();
...
impl DefaultKernelInvocable for MatmulGemvQkvFusedRmsnormOp {
    type Args<'a, T: TensorElement> = (
        &'a Tensor<T>,
        &'a Tensor<T>,
        (&'a QuantizedTensor<'a>, &'a QuantizedTensor<'a>, &'a QuantizedTensor<'a>),
    ...
    fn function_id() -> Option<KernelFunction> {
        Some(KernelFunction::MatmulGemvQkvFusedRmsnorm)
    }
    ...
    fn new<'a, T: TensorElement>(
        ...
        if T::DTYPE != Dtype::F16 {
            return Err(MetalError::UnsupportedDtype {
                operation: "MatmulGemvQkvFusedRmsnorm",
                dtype: T::DTYPE,
            });
        }
        let (x, gamma, (wq, wk, wv), (bias_q_opt, bias_k_opt, bias_v_opt)) = args;
        let x_dims = x.dims();
        if x_dims.len() != 2 || x_dims[0] != 1 {
            return Err(MetalError::InvalidShape("x must be [1,K]".into()));
        }
        let k = x_dims[1];
        if gamma.dims() != [k] {
            return Err(MetalError::InvalidShape("gamma must be [K]".into()));
        }
        ...
        let pipeline = pipeline.ok_or_else(|| MetalError::PipelineCreationFailed("MatmulGemvQkvFusedRmsnormOp".to_string()))?;

        let mut profiler_label = if crate::profiling_state::get_profiling_state() {
            ctx.take_gpu_scope()
                .unwrap_or_else(|| crate::context::GpuProfilerLabel::fallback("gemv_qkv_fused_rmsnorm"))
        } else {
            crate::context::GpuProfilerLabel::fallback("gemv_qkv_fused_rmsnorm")
        };
        profiler_label.op_name = format!("{}/matmul/gemv_qkv_fused_rmsnorm (Q)", profiler_label.op_name);
        profiler_label.backend = "gemv_qkv_fused_rmsnorm".to_string();
        if crate::profiling_state::get_profiling_state() {
            let mut data = rustc_hash::FxHashMap::default();
            data.insert("op".to_string(), "matmul".to_string());
            data.insert("backend".to_string(), "gemv_qkv_fused_rmsnorm".to_string());
            data.insert("batch".to_string(), "1".to_string());
            data.insert("m".to_string(), "1".to_string());
            data.insert("k".to_string(), k.to_string());
            data.insert("nq".to_string(), nq.to_string());
        ...
        let op: FusedQkvRmsnormOp<T> = FusedQkvRmsnormOp {
            pipeline,
            x: x.clone(),
            gamma: gamma.clone(),
            wq,
