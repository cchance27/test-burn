...
fn execute_swiglu_logic<T: TensorElement>(
    ...
    let (gate_temp, gate_leading_stride, up_temp, up_leading_stride) = if let Some(fused_weight) = fused_gate_up {
        ...
        let cache_for_fused = cache.as_deref_mut();
        ...
        let cache_for_gate = cache.as_deref_mut();
        ...
        let cache_for_up = cache.as_deref_mut();
    ...
    let ffn_temp = if hidden_cols == ffn_down_rows {
        ...
        let cache_for_down = cache.as_deref_mut();
        ...
        let cache_for_down_t = cache.as_deref_mut();
    ...
    let ffn_out = ctx.call::<BroadcastElemwiseAddInplaceOp>((ffn_temp, ffn_down_bias.clone()), cache)?;
