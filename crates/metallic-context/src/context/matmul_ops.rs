...
type QkvTuple<T> = (Tensor<T>, Tensor<T>, Tensor<T>);
type QkvResult<T> = Result<QkvTuple<T>, MetalError>;
...
impl<T: TensorElement> Context<T> {
    ...
    pub fn matmul(
        ...
        match b_any {
            TensorType::Dense(bd) => self.matmul_dense(a, bd, transpose_a, transpose_b, bias, alpha_beta, cache),
            TensorType::DenseCanonical(bc) => self.matmul_dense_canonical(a, bc, transpose_a, transpose_b, bias, alpha_beta, cache),
            TensorType::Quant(qrhs) => self.matmul_quant(a, qrhs, transpose_a, transpose_b, bias, alpha_beta, cache),
        }
    }
    ...
    pub fn matmul_dense_canonical(
        ...
        if dims.m == 1 {
            ...
            return self.launch_gemv(a, rhs, transpose_b, bias, alpha_beta, cache);
        }
        ...
        if self.should_use_f16_canonical_rows16(&dims, transpose_a, transpose_b) {
            ...
            return self.call::<MatmulF16CanonicalRows16Op>((a, b, bias), cache);
        }

        emit_matmul_backend_selected("dense_canonical", Some(dims), transpose_a, transpose_b, "F16Canonical", "heuristic");
        self.call::<MatmulF16CanonicalOp>((a, b, bias), cache)
    }

    pub fn qkv(&mut self, x_flat: &Tensor<T>, weights: QkvWeights<'_, T>) -> QkvResult<T> {
    ...
    fn matmul_dense(
        ...
        match self.forced_matmul_backend {
            InternalMatMulBackendOverride::Force(MatMulBackend::Mlx) => {
                ...
                return self.matmul_dense_mlx(a, b, transpose_a, transpose_b, bias, alpha_beta, cache);
            }
            InternalMatMulBackendOverride::Force(MatMulBackend::Gemv) => {
                if can_gemv {
                    ...
                    return self.launch_gemv(a, TensorType::Dense(b), transpose_b, bias, alpha_beta, cache);
                }
        ...
        self.matmul_dense_mlx(a, b, transpose_a, transpose_b, bias, alpha_beta, cache)
    }
    ...
    fn launch_gemv(
        ...
        if let Some(ep) = alpha_beta {
            ...
            self.call::<MatmulGemvOp>((a, rhs, transpose_right, bias), cache)
        }
    }
            ...
            QuantizedTensor::Q8_0(q8) => {
                ...
                if transpose_b {
                    ...
                    if self.should_use_q8_canonical_rows16(&dims, transpose_a) {
                        ...
                        return self.call::<MatmulQ8CanonicalRows16Op>((a, q8, bias), cache);
                    }
                    if self.should_use_q8_canonical(&dims, transpose_a) {
                        ...
                        return self.call::<MatmulQ8CanonicalOp>((a, q8, bias), cache);
                    }
    ...
    fn qkv_quant(
        ...
        if wq.first() != Some(&k) || wk.first() != Some(&k) || wv.first() != Some(&k) {
