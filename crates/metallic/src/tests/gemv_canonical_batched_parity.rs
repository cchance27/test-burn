...
use crate::{
    MetalError, compound::stages::Layout, foundry::{Foundry, storage::Pooled, tensor::Tensor as FoundryTensor}, metals::gemv::step::{GemvStrategy, GemvV2Args, get_gemv_v2_kernel_f16, warp_dispatch_config_2d}, tensor::{F16, TensorInit}, types::TensorArg
};
...
fn test_gemv_canonical_batched_parity_m4() -> Result<(), MetalError> {
    ...
    // Upload tensors
    let x = FoundryTensor::<F16, Pooled>::new(&mut foundry, vec![m, k], TensorInit::CopyFrom(&x_f16))?;
    let w = FoundryTensor::<F16, Pooled>::new(
        &mut foundry,
        vec![blocks_per_k * n * weights_per_block],
        TensorInit::CopyFrom(&w_canon_f16),
    )?;
    let y = FoundryTensor::<F16, Pooled>::new(&mut foundry, vec![m, n], TensorInit::Uninitialized)?;
    ...
    let y_arg = TensorArg::from_tensor(&y);
    let args = GemvV2Args {
        weights: TensorArg::from_tensor(&w),
        scale_bytes: TensorArg::from_tensor(&w), // Dummy for F16
        input: TensorArg::from_tensor(&x),
        output: y_arg.clone(),
        k_dim: k as u32,
        n_dim: n as u32,
        ...
        has_bias: 0,
        alpha: 1.0,
        residual: TensorArg::from_tensor(&y),
        has_residual: 0,
        beta: 0.0,
    };
