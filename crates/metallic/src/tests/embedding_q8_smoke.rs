...
use crate::{
    ...
    }, metals::embedding::step::EmbeddingStep, tensor::{F16, TensorInit, U8, U32}, types::TensorArg
};
...
fn test_embedding_q8_smoke() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for feat in 0..d_model {
        table[0 * d_model + feat] = feat as i8 as u8; // tok0: 0..31
        table[1 * d_model + feat] = (2 * feat) as i8 as u8; // tok1: 0..62
    }
    ...
    let step = EmbeddingStep {
        ...
        output: "out".into(),
        params: crate::metals::embedding::EmbeddingParams {
