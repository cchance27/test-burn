...
use crate::{
    MetalError, compound::stages::Layout, foundry::{Foundry, storage::Pooled, tensor::Tensor as FoundryTensor}, metals::gemv::{GemvStrategy, GemvV2Args, get_gemv_v2_kernel_f16, warp_dispatch_config}, tensor::{F16, TensorInit}, types::TensorArg
};
...
fn test_gemv_v2_sdpa_foundry_av() -> Result<(), MetalError> {
    ...
    let kernel = get_gemv_v2_kernel_f16(Layout::ColMajor, GemvStrategy::Vectorized);
    let dispatch = warp_dispatch_config(n as u32);
