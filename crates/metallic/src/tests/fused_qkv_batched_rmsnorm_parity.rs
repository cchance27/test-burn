...
use crate::{
    foundry::{
        Foundry, spec::{DynamicValue, Step, TensorBindings}, storage::Pooled, tensor::Tensor as FoundryTensor
    }, metals::gemv::qkv_step::FusedQkvStep, tensor::{F16, TensorInit}
};
...
fn test_fused_qkv_batched_rmsnorm_parity_m2() -> Result<(), Box<dyn std::error::Error>> {
    ...
    bindings.insert("hidden".to_string(), crate::types::TensorArg::from_tensor(&hidden));
    bindings.insert("gamma".to_string(), crate::types::TensorArg::from_tensor(&gamma));
    bindings.insert("w_q".to_string(), crate::types::TensorArg::from_tensor(&w_q));
    bindings.insert("w_k".to_string(), crate::types::TensorArg::from_tensor(&w_k));
    bindings.insert("w_v".to_string(), crate::types::TensorArg::from_tensor(&w_v));
    bindings.insert("q".to_string(), crate::types::TensorArg::from_tensor(&q_out));
    bindings.insert("k".to_string(), crate::types::TensorArg::from_tensor(&k_out));
    bindings.insert("v".to_string(), crate::types::TensorArg::from_tensor(&v_out));
    ...
    let fused = FusedQkvStep {
        ...
        strategy: crate::metals::gemv::step::GemvStrategy::Vectorized,
    };
