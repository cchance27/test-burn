...
fn test_batched_pipeline_parity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use crate::metals::gemv::step::GemvStrategy; // Import GemvStrategy
    use crate::{foundry::spec::DynamicValue, metals::gemv::qkv_step::*}; // Import DynamicValue
    ...
    let sdpa_step = SdpaMaterializedStep {
        ...
        causal: true,
        query_offset: DynamicValue::Variable("position_offset".into()),
        n_heads: DynamicValue::Literal(n_heads as u32),
        head_dim: DynamicValue::Literal(head_dim as u32),
        kv_seq_len: DynamicValue::Variable("kv_seq_len".into()),
        m: DynamicValue::Variable("m".into()),
        kv_head_major: true,
    };
