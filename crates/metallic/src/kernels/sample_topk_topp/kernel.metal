...
inline void simd_reduce_argmax_pair_unrolled(thread float &best_val, thread uint &best_idx, thread uint &best_lane, uint simd_width) {
    // Unrolled pairwise argmax reduction with deterministic tie-break on index.
    if (simd_width >= 32u) {
        float other_val = simd_shuffle_down(best_val, 16u);
        uint  other_idx = simd_shuffle_down(best_idx, 16u);
        uint  other_ln  = simd_shuffle_down(best_lane, 16u);
...
// f32 variant
kernel void sample_topk_fused_f32(
