...
impl DefaultKernelInvocable for MatMulMpsAlphaBetaOp {
    ...
    fn new<'a, T: TensorElement>(
        ...
        // For alpha-beta operations, use appropriate op type
        let op_type = if matmul_result_view.batch > 1 {
            "matmul_alpha_beta_cache"
        } else {
            "matmul_alpha_beta"
        };
        ...
        profiler_label.op_name = format!("{}/{}/mps", profiler_label.op_name, op_type);
        ...
        if crate::profiling_state::get_profiling_state() {
            ...
            data.insert("batch".to_string(), matmul_result_view.batch.to_string());
