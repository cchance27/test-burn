...
fn test_mlx_kernel_matches_mps_basic_matmul() -> Result<(), MetalError> {
    ...
    let out_mps = ctx_mps.matmul(&a_mps, &TensorType::Dense(&b_mps), false, false, None, None, None)?;
    ctx_mps.synchronize();
    let out_mlx = ctx_mlx.matmul(&a_mlx, &TensorType::Dense(&b_mlx), false, false, None, None, None)?;
    ctx_mlx.synchronize();
...
fn test_mlx_kernel_matches_mps_with_transpositions() -> Result<(), MetalError> {
    ...
    let out_left_mps = ctx_mps.matmul(&a_left_mps, &TensorType::Dense(&b_left_mps), true, false, None, None, None)?;
    ctx_mps.synchronize();
    let out_left_mlx = ctx_mlx.matmul(&a_left_mlx, &TensorType::Dense(&b_left_mlx), true, false, None, None, None)?;
    ctx_mlx.synchronize();
    ...
    let out_right_mps = ctx_mps.matmul(&a_right_mps, &TensorType::Dense(&b_right_mps), false, true, None, None, None)?;
    ctx_mps.synchronize();
    let out_right_mlx = ctx_mlx.matmul(&a_right_mlx, &TensorType::Dense(&b_right_mlx), false, true, None, None, None)?;
    ctx_mlx.synchronize();
    ...
    let out_both_mps = ctx_mps.matmul(&a_both_mps, &TensorType::Dense(&b_both_mps), true, true, None, None, None)?;
    ctx_mps.synchronize();
    let out_both_mlx = ctx_mlx.matmul(&a_both_mlx, &TensorType::Dense(&b_both_mlx), true, true, None, None, None)?;
    ctx_mlx.synchronize();
...
fn test_mlx_kernel_matches_mps_non_contiguous_views() -> Result<(), MetalError> {
    ...
    let out_mps = ctx_mps.matmul(&a_view_mps, &TensorType::Dense(&b_view_mps), false, false, None, None, None)?;
    ctx_mps.synchronize();
    let out_mlx = ctx_mlx.matmul(&a_view_mlx, &TensorType::Dense(&b_view_mlx), false, false, None, None, None)?;
    ctx_mlx.synchronize();
...
fn test_mlx_fused_bias_add_matches_mps() -> Result<(), MetalError> {
    ...
    // MPS path: matmul + add
    let out_mps_matmul = ctx_mps.matmul(&a_mps, &TensorType::Dense(&b_mps), false, false, None, None, None)?;
    let out_mps = ctx_mps.call::<BroadcastElemwiseAddInplaceOp>((out_mps_matmul, bias_mps), None)?;
    ctx_mps.synchronize();

    // MLX path: fused matmul_bias_add
    let out_mlx = ctx_mlx.matmul(&a_mlx, &TensorType::Dense(&b_mlx), false, false, Some(&bias_mlx), None, None)?;
    ctx_mlx.synchronize();
