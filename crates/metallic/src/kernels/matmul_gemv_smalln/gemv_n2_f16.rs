...
pub struct MatmulGemvSmallN2Op;

// Internal struct that holds data for the `Operation` trait.
struct MatmulGemvSmallN2<T: TensorElement> {
    a: Tensor<T>,
    b: Tensor<T>,
    out: Tensor<T>,
    m: u32,
...
impl DefaultKernelInvocable for MatmulGemvSmallN2Op {
    type Args<'a, T: TensorElement> = (&'a Tensor<T>, &'a Tensor<T>);

    fn function_id() -> Option<KernelFunction> {
        Some(KernelFunction::MatmulGemvSmallN2)
    }
    ...
    fn new<'a, T: TensorElement>(
        ...
        if n != 2 {
            return Err(MetalError::InvalidShape(format!("MatmulGemvSmallN2Op requires N=2, got N={}", n)));
        }
        ...
        let profiler_label = ctx
            .take_gpu_scope()
            .unwrap_or_else(|| GpuProfilerLabel::fallback("matmul_gemv_small_n2_op"));

        let op = MatmulGemvSmallN2 {
            a: a.clone(),
            b: b.clone(),
            out: out.clone(),
            m: m as u32,
