...
pub struct MatmulGemvSmallN4Op;

// Internal struct that holds data for the `Operation` trait.
struct MatmulGemvSmallN4<T: TensorElement> {
    a: Tensor<T>,
    b: Tensor<T>,
    out: Tensor<T>,
    m: u32,
...
impl DefaultKernelInvocable for MatmulGemvSmallN4Op {
    type Args<'a, T: TensorElement> = (&'a Tensor<T>, &'a Tensor<T>);

    fn function_id() -> Option<KernelFunction> {
        Some(KernelFunction::MatmulGemvSmallN4)
    }
    ...
    fn new<'a, T: TensorElement>(
        ...
        if n != 4 {
            return Err(MetalError::InvalidShape(format!("MatmulGemvSmallN4Op requires N=4, got N={}", n)));
        }
        ...
        let profiler_label = ctx
            .take_gpu_scope()
            .unwrap_or_else(|| GpuProfilerLabel::fallback("matmul_gemv_small_n4_op"));

        let op = MatmulGemvSmallN4 {
            a: a.clone(),
            b: b.clone(),
            out: out.clone(),
            m: m as u32,
