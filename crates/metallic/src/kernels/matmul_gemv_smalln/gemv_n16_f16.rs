...
struct MatmulGemvSmallN16<T: TensorElement> {
    a: Tensor<T>,
    b: Tensor<T>,
    out: Tensor<T>,
...
impl KernelInvocable for MatmulGemvSmallN16Op {
    ...
    fn function_id() -> Option<KernelFunction> {
        Some(KernelFunction::MatmulGemvSmallN16)
    }
    ...
    fn new<'a, T: TensorElement>(
        ...
        if n != 16 {
            return Err(MetalError::InvalidShape(format!("MatmulGemvSmallN16Op requires N=16, got N={}", n)));
        }
        ...
        let profiler_label = ctx
            .take_gpu_scope()
            .unwrap_or_else(|| GpuProfilerLabel::fallback("matmul_gemv_small_n16_op"));

        let op = MatmulGemvSmallN16 {
            a: a.clone(),
            b: b.clone(),
            out: out.clone(),
...
impl<T: TensorElement> Operation for MatmulGemvSmallN16<T> {
