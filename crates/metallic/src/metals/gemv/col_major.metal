...
// Layout: element (col, k) at matrix[batch_offset + col * stride_w + k].
...
// Column-Major dispatch constants
...
// Core Column-Major GEMV implementation.
template<typename Policy, bool HasBias>
void run_gemv_col_major_core(
    ...
    // Base for this column (col-major: col * stride_w gives start, K is contiguous)
    const ulong col_base = matrix_batch_offset + (ulong)logical_col * params->stride_w;
    ...
    while (k_base + K_CHUNK_SIZE <= K) {
        ...
        float w[8];
        Policy::template load_weights<8>(matrix, col_base + (ulong)k, w);
        
        // Load scale for this block (k is multiple of 8, block size is 32)
        // Global block index = logical_col * blocks_per_k + (k / wpb)
        const uint wpb = params->weights_per_block;
        const uint blocks_per_k = params->blocks_per_k;
        half scale = Policy::load_scale(scale_bytes, (ulong)logical_col * blocks_per_k + (k / wpb));
...
#ifndef FUSED_KERNEL
[[kernel]] void gemv_col_major_f16(
    ...
    if (has_bias != 0) {
        run_gemv_col_major_core<PolicyF16, true>(
            matrix, vector_x, result_y, params, bias, residual, alpha, beta, gid, lid, scale_bytes
        );
    } else {
        run_gemv_col_major_core<PolicyF16, false>(
            matrix, vector_x, result_y, params, bias, residual, alpha, beta, gid, lid, scale_bytes
        );
    }
}
