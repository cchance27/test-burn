...
use crate::{
    MetalError, compound::stages::{Layout, Quantization}, foundry::{
        Foundry, spec::{CompiledStep, DynamicValue, FastBindings, Ref, Step, SymbolTable, TensorBindings}
    }, metals::{gemm::step::GemmV2Step, gemv::step::GemvV2Step}
};
...
impl Step for MatMulStep {
    fn execute(&self, foundry: &mut Foundry, bindings: &mut TensorBindings) -> Result<(), MetalError> {
        ...
        use crate::foundry::spec::{FastBindings, SymbolTable};
    ...
    fn compile(&self, bindings: &mut TensorBindings, symbols: &mut SymbolTable) -> Vec<Box<dyn CompiledStep>> {
        ...
        let gemv_compiled: Vec<Box<dyn CompiledStep>> = if use_canonical {
            ...
            let compiled = gemv_step.compile(bindings, symbols);
