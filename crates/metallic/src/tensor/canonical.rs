...
impl<T: TensorElement> CanonicalF16Tensor<T> {
    ...
    pub fn write_from_kn_tensor(&mut self, src: &Tensor<T>, dst_col_offset: usize) -> Result<(), MetalError> {
        self.write_from_kn_slice(src.as_slice(), src.dims(), dst_col_offset)
    }
    ...
    /// Swizzle KN row-major weights (rows = K/in, cols = N/out) into canonical k-block-major layout.
    pub fn write_from_kn_slice(&mut self, src: &[T::Scalar], src_dims: &[usize], dst_col_offset: usize) -> Result<(), MetalError> {
        if T::DTYPE != Dtype::F16 {
            return Err(MetalError::UnsupportedDtype {
                operation: "CanonicalF16Tensor::write_from_kn_slice",
                dtype: T::DTYPE,
            });
        }
        if src_dims.len() != 2 {
        ...
        let out_features = if src_dims[0] == k {
            src_dims[1]
        } else {
            return Err(MetalError::InvalidShape(format!(
                "Source dims {:?} do not match expected K={} for KN layout",
                src_dims, k
            )));
        };
            ...
            for block in 0..blocks_per_k {
                ...
                if remaining >= weights_per_block {
                    for i in 0..weights_per_block {
                        let k_idx = k_base + i;
                        let src_idx = k_idx * out_features + out_idx;
                        dst[dst_base + i] = src[src_idx];
                    }
                } else {
                    if remaining > 0 {
