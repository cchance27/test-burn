...
fn run_parity_test(cfg: TestConfig) {
    ...
    // Legacy Kernel
    // =========================================================================
    let table_legacy = Tensor::<F16>::new(
        vec![cfg.vocab_size, cfg.d_model],
        LegacyStorage::Pooled(&mut ctx),
        TensorInit::CopyFrom(&table_data),
    )
    .unwrap();
    ...
    let mut indices_legacy =
        Tensor::<U32Type>::from_existing_buffer(buf, vec![total_tokens], U32Type::DTYPE, &ctx.device, &ctx.command_queue, 0, true).unwrap();
    ...
    // =========================================================================
    let table_foundry =
        FoundryTensor::<F16, Pooled>::new(&mut foundry, vec![cfg.vocab_size * cfg.d_model], TensorInit::CopyFrom(&table_data)).unwrap();
    let indices_foundry =
