...
use half::f16;
use metallic::{
    MetalError, foundry::{Foundry, model::ModelBuilder, spec::ModelSpec}, gguf::{GGUFDataType, GGUFFile, model_loader::GGUFModelLoader, tensor_info::GGUFRawTensor}, types::KernelArg as _
};
...
fn gguf_tensor_f32(
    model: &metallic::gguf::model_loader::GGUFModel,
    name: &str,
    ...
    for (key, expected_n, expected_k) in weights {
        if let Some(name) = arch.tensor_names.resolve(key, Some(layer), &available) {
            let tensor = gguf_model
                .get_tensor(&name)
                .ok_or_else(|| MetalError::InvalidShape(format!("GGUF tensor '{name}' not found")))?;
