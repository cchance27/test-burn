...
use metallic::{
    ...
    }, tensor::{F16, TensorInit}, types::{DispatchConfig, GridSize, TensorArg, ThreadgroupSize}
};
...
fn dequantize_q8(data: &[u8], scales: &[u8], k: usize, n: usize) -> Vec<f16> {
    ...
    let w_per_blk = 32;
    let blocks_per_k = (k + w_per_blk - 1) / w_per_blk;
    for r in 0..n {
...
fn test_qkv_parity() {
    ...
    for i in 0..(n_dim * k_dim) {
        w_q_data[i] = f16::from_f32(rng.random_range(-0.1..0.1));
    }
    ...
    for i in 0..n_dim {
        b_q_data[i] = f16::from_f32(rng.random_range(-0.1..0.1));
    }
    for i in 0..n_kv {
    ...
    let warps_per_tg = 8;
    let num_groups = (n_dim.max(n_kv) + warps_per_tg - 1) / warps_per_tg;
    ...
    // Bench Legacy
    let mut ctx = metallic::Context::<metallic::tensor::F16>::new().unwrap();
    let lx = metallic::tensor::Tensor::<metallic::tensor::F16>::new(
        vec![1, k_dim],
        metallic::tensor::TensorStorage::Pooled(&mut ctx),
        metallic::tensor::TensorInit::CopyFrom(&x_data),
    )
    .unwrap();
    let lgamma = metallic::tensor::Tensor::<metallic::tensor::F16>::new(
        vec![k_dim],
        metallic::tensor::TensorStorage::Pooled(&mut ctx),
        metallic::tensor::TensorInit::CopyFrom(&gamma_data),
    )
    .unwrap();
    ...
    let lwq_q =
        metallic::tensor::quantized::QuantizedQ8_0Tensor::from_split_bytes_in_context(vec![k_dim, n_dim], &lwq_data, &lwq_scales, &mut ctx)
            .unwrap();
    let lwk_q =
        metallic::tensor::quantized::QuantizedQ8_0Tensor::from_split_bytes_in_context(vec![k_dim, n_kv], &lwk_data, &lwk_scales, &mut ctx)
            .unwrap();
    let lwv_q =
        metallic::tensor::quantized::QuantizedQ8_0Tensor::from_split_bytes_in_context(vec![k_dim, n_kv], &lwv_data, &lwv_scales, &mut ctx)
            .unwrap();

    let lb_q = metallic::tensor::Tensor::<metallic::tensor::F16>::new(
        vec![n_dim],
        metallic::tensor::TensorStorage::Pooled(&mut ctx),
        metallic::tensor::TensorInit::CopyFrom(&b_q_data),
    )
    .unwrap();
    let lb_k = metallic::tensor::Tensor::<metallic::tensor::F16>::new(
        vec![n_kv],
        metallic::tensor::TensorStorage::Pooled(&mut ctx),
        metallic::tensor::TensorInit::CopyFrom(&b_k_data),
    )
    .unwrap();
    let lb_v = metallic::tensor::Tensor::<metallic::tensor::F16>::new(
        vec![n_kv],
        metallic::tensor::TensorStorage::Pooled(&mut ctx),
        metallic::tensor::TensorInit::CopyFrom(&b_v_data),
    )
    .unwrap();
    ...
    for _ in 0..iters {
        let _ = ctx
            .call::<metallic::kernels::matmul_gemv_qkv_fused::MatmulGemvQkvFusedRmsnormOp>(
                (
                    &lx,
                    &lgamma,
                    (
                        &metallic::tensor::QuantizedTensor::Q8_0(&lwq_q),
                        &metallic::tensor::QuantizedTensor::Q8_0(&lwk_q),
                        &metallic::tensor::QuantizedTensor::Q8_0(&lwv_q),
                    ),
