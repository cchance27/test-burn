...
use half::f16;
use metallic::{
    Context, F16Element, MetalError, foundry::{Foundry, storage::Pooled, tensor::Tensor as FoundryTensor}, kernels::rope::RoPEOp, metals::{
...
fn test_sdpa_v2_vs_context_rope_sdpa() -> Result<(), MetalError> {
    ...
    let q_roped = ctx.call::<RoPEOp>(
        (q_ctx, cos_ctx.clone(), sin_ctx.clone(), head_dim as u32, 1, (kv_len - 1) as u32),
        None,
    )?;

    // Apply RoPE to K (position = 0..kv_len)
    let k_roped = ctx.call::<RoPEOp>((k_ctx, cos_ctx, sin_ctx, head_dim as u32, kv_len as u32, 0), None)?;

    ctx.synchronize();

    // Debug: Compare the pre-roped K values
    let k_roped_ctx = k_roped.try_to_vec()?;
    ...
    let out_ctx = ctx.scaled_dot_product_attention(&q_roped, &k_roped, &v_ctx, true)?;
