...
use half::f16;
use metallic::{
    Context, Dtype, Tensor as LegacyTensor, compound::stages::Quantization, foundry::{Foundry, storage::Pooled, tensor::Tensor as FoundryTensor}, kernels::matmul_mlx::MatMulMlxOp, metals::{
        gemm::step::{GemmParams, GemmV2Args, gemm_dispatch_config, get_gemm_kernel}, mma::stages::TileConfig
    }, tensor::{F16, QuantizedTensor, TensorInit, TensorStorage, TensorType, quantized::QuantizedQ8_0Tensor}, types::{MetalBuffer, TensorArg}
};
use rand::{Rng, rng};
...
fn run_mlx_gemm_f16(
    ...
    let mut ctx = Context::<F16>::new().expect("Failed to create context");

    let a_rows = if trans_a { k } else { m };
    let a_cols = if trans_a { m } else { k };
    let a_legacy = LegacyTensor::<F16>::new(vec![a_rows, a_cols], TensorStorage::Dedicated(&ctx), TensorInit::CopyFrom(a_data)).unwrap();

    let b_rows = if trans_b { n } else { k };
    let b_cols = if trans_b { k } else { n };
    let b_legacy = LegacyTensor::<F16>::new(vec![b_rows, b_cols], TensorStorage::Dedicated(&ctx), TensorInit::CopyFrom(b_data)).unwrap();
...
fn run_mlx_gemm_q8(
    ...
    let res = ctx.call::<MatMulMlxOp>(
        (
            &a_legacy,
            TensorType::Quant(QuantizedTensor::Q8_0(b_q8)),
            None,
            None,
            trans_a,
            trans_b,
...
fn run_gemm_v2_parity_test(cfg: GemmTestConfig) {
    ...
    // Create tensors
    let a = FoundryTensor::<F16, Pooled>::new(&mut foundry, a_dims, TensorInit::CopyFrom(&a_data)).unwrap();

    // We need a Context<F16> to create the quantized tensor
    let ctx_f16 = Context::<F16>::new().unwrap();

    let (b_quantized, b_f16) = if cfg.quant_b == Quantization::Q8 {
        ...
        let b = FoundryTensor::<F16, Pooled>::new(&mut foundry, b_dims, TensorInit::CopyFrom(&b_data)).unwrap();
        (None, Some(b))
    };

    let output = FoundryTensor::<F16, Pooled>::new(&mut foundry, vec![cfg.m, cfg.n], TensorInit::Uninitialized).unwrap();
    ...
    let (b_arg, b_scales_arg) = if let Some(q8) = &b_quantized {
        let b_data_arg = TensorArg::from_buffer(
            MetalBuffer(q8.data.buf.clone()),
            Dtype::U8,
            q8.data.dims.clone(),
            q8.data.strides.clone(),
        );
        let b_scales_arg = TensorArg::from_buffer(
            MetalBuffer(q8.scales.buf.clone()),
            Dtype::F16, // Scales are F16
