use half::f16;
use metallic::{
    compound::CompoundKernel, foundry::{Foundry, storage::Pooled, tensor::Tensor}, metals::swiglu::{SwigluParamsResolved, stages::SwigluStage, step::SwigluArgs}, tensor::{F16, TensorInit}, types::{DispatchConfig, GridSize, TensorArg, ThreadgroupSize}
};
use rand::Rng;
...
fn test_swiglu_parity() {
    ...
    let kernel = Box::leak(Box::new(
        CompoundKernel::new("fused_swiglu")
            .main(SwigluStage::new(params.clone()))
    ...
    let num_groups = (vectorized_threads + threads_per_group - 1) / threads_per_group;
