...
fn run_parity_test(cfg: TestConfig) {
    ...
    .unwrap();
    let cos_legacy = Tensor::<F16>::new(
        vec![max_seq, half_dim],
        LegacyStorage::Pooled(&mut ctx),
        TensorInit::CopyFrom(&cos_data),
    )
    .unwrap();
    let sin_legacy = Tensor::<F16>::new(
        vec![max_seq, half_dim],
        LegacyStorage::Pooled(&mut ctx),
    ...
    // Foundry Kernel
    // =========================================================================
    let input_foundry = FoundryTensor::<F16, Pooled>::new(&mut foundry, vec![total_elements], TensorInit::CopyFrom(&input_data)).unwrap();
