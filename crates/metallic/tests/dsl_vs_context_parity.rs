...
use half::f16;
use metallic::{
    Context, F16Element, MetalError, Tensor, TensorElement, context::{QkvWeights, RepeatKvWorkspaceKind}, foundry::{Foundry, model::ModelBuilder, spec::TensorBindings}, kernels::{
    ...
    }, models::Qwen25, tensor::{QuantizedTensor, TensorType}, types::KernelArg
};
...
fn legacy_forward_blocks_only<T: TensorElement>(
    ...
    for (layer_idx, block) in model.blocks.iter().enumerate() {
        ...
        let k_repeated = ctx.call::<RepeatKvHeadsOp>(
            (
                k_heads_after_rope,
                group_size as u32,
                batch as u32,
        ...
        let v_repeated = ctx.call::<RepeatKvHeadsOp>(
            (
                v_heads,
                group_size as u32,
                batch as u32,
...
fn legacy_sdpa_layer0<T: TensorElement>(model: &Qwen25<T>, input: &Tensor<T>, ctx: &mut Context<T>) -> Result<Tensor<T>, MetalError> {
    ...
    let x_normed_attn = ctx.call::<RMSNormOp>((input.clone(), block.attn_norm_gamma.clone(), d_model as u32), None)?;
    let m = batch * seq;
    let x_flat = x_normed_attn.reshape(vec![m, d_model])?;
...
fn legacy_layer0_outputs<T: TensorElement>(
    ...
    let residual_1 = resid_attn.add_elem(&proj_out, ctx)?;
    let resid_mlp = residual_1.clone();
    let x_normed_mlp = ctx.call::<RMSNormOp>((residual_1, block.ffn_norm_gamma.clone(), d_model as u32), None)?;
    let x_normed_mlp_flat = x_normed_mlp.reshape(vec![m, d_model])?;

    let (gate_proj, _up_proj, swiglu_out, ffn_output_flat) = if f16_canonical {
        if let (Some(g), Some(u), Some(d)) = (&block.ffn_gate_canon, &block.ffn_up_canon, &block.ffn_down_canon) {
            let gate = ctx.matmul(&x_normed_mlp_flat, &TensorType::DenseCanonical(g), false, false, None, None, None)?;
            let up = ctx.matmul(&x_normed_mlp_flat, &TensorType::DenseCanonical(u), false, false, None, None, None)?;
            let gate_bias = ctx.call::<BroadcastElemwiseAddOp>((gate.clone(), block.ffn_gate_bias.clone()), None)?;
            let up_bias = ctx.call::<BroadcastElemwiseAddOp>((up.clone(), block.ffn_up_bias.clone()), None)?;
            let silu_out = ctx.call::<SiluOp>(gate_bias, None)?;
            let mul_out = ctx.call::<ElemwiseMulOp>((silu_out, up_bias), None)?;
            let down = ctx.matmul(&mul_out, &TensorType::DenseCanonical(d), false, false, None, None, None)?;
            ...
            let gate = ctx.matmul(
                ...
                false,
                transpose_for(block.ffn_gate.as_ref().unwrap().dims(), d_model, "ffn_gate")?,
                None,
                None,
                None,
            )?;
            let up = ctx.matmul(
                ...
                false,
                transpose_for(block.ffn_up.as_ref().unwrap().dims(), d_model, "ffn_up")?,
                None,
                None,
                None,
            )?;
            ...
            let down = ctx.matmul(
                ...
                false,
                transpose_for(block.ffn_down.as_ref().unwrap().dims(), block.ffn_gate_bias.len(), "ffn_down")?,
                None,
                None,
                None,
            )?;
    ...
    let ffn_output_flat = ctx.call::<BroadcastElemwiseAddInplaceOp>((ffn_output_flat, block.ffn_down_bias.clone()), None)?;
    let ffn_out = ffn_output_flat.reshape(vec![batch, seq, d_model])?;
    let hidden = resid_mlp.add_elem(&ffn_out, ctx)?;
...
fn test_dsl_vs_context_embedding_parity() -> Result<(), MetalError> {
    ...
    // =========================================================================
    let legacy_embedded = legacy_model.embed(&tokens, &mut ctx)?;
    ctx.synchronize();
    ...
    if !arch.forward.is_empty() {
        ...
        embedding_step.execute(&mut foundry, &mut bindings)?;
    }

    // Read back the hidden tensor
    let hidden_arg = bindings.get("hidden")?;
...
fn test_dsl_vs_context_generation_greedy_parity() -> Result<(), MetalError> {
    ...
    for layer_idx in 0..legacy_model.config.n_layers {
        ctx.alloc_kv_cache(layer_idx, kv_capacity, legacy_n_heads, kv_head_dim)?;
    }
        ...
        std::env::var("METALLIC_PARITY_COMPARE_V_CACHE_LAYERS")
            ...
            .and_then(|raw| {
                ...
                for chunk in raw.split(',') {
                    ...
                    let layer = chunk.parse::<usize>().ok()?;
    ...
    let mut run_legacy = |token: u32, pos: usize| -> Result<(Vec<f16>, Vec<f16>, Option<Vec<(usize, Vec<f16>)>>), MetalError> {
        let embedded = legacy_model.embed(&[token], &mut ctx)?;
        let (hidden, _) = legacy_model.forward_step(&embedded, pos, &mut ctx)?;
        let logits = legacy_model.output(&hidden, &mut ctx)?;
        ...
        if pos == 0 {
            ...
            if let Some(layers) = cache_layer_list.as_ref() {
                ...
                for &layer_idx in layers {
                    ...
                    let legacy_view = ctx.kv_cache_history_view(&cache, pos + 1)?.0;
                    snapshots.push((layer_idx, tensor_to_f16_vec_strided(&legacy_view)?));
                }
    ...
    for (pos, token) in tokens.iter().copied().enumerate() {
        let (legacy_logits, legacy_hidden, legacy_v_cache_layers) = run_legacy(token, pos)?;
        ...
        if pos == 0 {
            ...
            if enable_legacy_v_cache_compare {
                if let (Ok(v_tensor), Ok(v_cache_tensor)) = (bindings.get("v_heads"), bindings.get("v_cache_0")) {
    ...
    let mut current_token = next_token.ok_or_else(|| MetalError::InvalidShape("Missing next token after prompt".to_string()))?;
    let mut pos = tokens.len();

    for step in 0..max_new_tokens {
        generated.push(current_token);

        let (legacy_logits, legacy_hidden, _legacy_v_cache) = run_legacy(current_token, pos)?;
        let (dsl_logits, dsl_hidden) = run_dsl_step(
            &mut foundry,
            &mut bindings,
            &input_buffer,
...
fn test_dsl_vs_context_pre_norm_parity() -> Result<(), MetalError> {
    ...
    let legacy_embedded = legacy_model.embed(&tokens, &mut ctx)?;
    let legacy_hidden_pre = legacy_forward_blocks_only(&legacy_model, &legacy_embedded, &mut ctx)?;
    ctx.synchronize();
    ...
    let dsl_hidden = bindings.get("hidden")?;
...
fn test_dsl_vs_context_sdpa_layer0_parity() -> Result<(), MetalError> {
    ...
    let legacy_embedded = legacy_model.embed(&tokens, &mut ctx)?;
    let legacy_attn_out = legacy_sdpa_layer0(&legacy_model, &legacy_embedded, &mut ctx)?;
    ctx.synchronize();
    ...
    let dsl_attn_out = bindings.get("attn_out")?;
...
fn test_dsl_vs_context_layer0_block_parity() -> Result<(), MetalError> {
    ...
    let input_tensor = metallic::types::TensorArg::from_buffer(input_buffer, metallic::tensor::Dtype::U32, vec![1], vec![1]);
    bindings.insert("input_ids".to_string(), input_tensor);
    ...
    let legacy_embedded = legacy_model.embed(&[token], &mut ctx)?;
    let legacy_out = legacy_layer0_outputs(&legacy_model, &legacy_embedded, &mut ctx)?;

    let ffn_norm_out = read_f16_buffer(&bindings.get("ffn_norm_out")?);
    let ffn_norm_gamma = read_f16_buffer(&bindings.get("layer.ffn_norm_0")?);
    let gate = read_f16_buffer(&bindings.get("gate")?);
    let swiglu_out = read_f16_buffer(&bindings.get("up")?);
    let proj_out = read_f16_buffer(&bindings.get("proj_out")?);
    let residual_1 = read_f16_buffer(&bindings.get("residual_1")?);
    let ffn_out = read_f16_buffer(&bindings.get("ffn_out")?);
    let hidden = read_f16_buffer(&bindings.get("hidden")?);
    ...
    let dsl_attn_out_full = read_f16_buffer(&bindings.get("attn_out")?);
    ...
    let dsl_q_proj = read_f16_buffer(&bindings.get("q")?);
    eprintln!(
    ...
    let dsl_q_heads = read_f16_buffer(&bindings.get("q_heads")?);
    let dsl_q_rot = read_f16_buffer(&bindings.get("q_rot")?);
    eprintln!(
    ...
    let dsl_k_expanded = read_f16_buffer(&bindings.get("k_expanded")?);
    ...
    let dsl_v_expanded = read_f16_buffer(&bindings.get("v_expanded")?);
    ...
    let q_weight = bindings.get("layer.attn_q_0")?;
    let q_bias = bindings.get("layer.attn_q_bias_0")?;
    ...
    let norm_out_data = read_f16_buffer(&bindings.get("norm_out")?);
    ...
    let legacy_q_bias_first5: Vec<f32> = legacy_q_bias_slice[..5.min(legacy_q_bias_slice.len())]
        ...
        .map(|x| x.to_f32())
        .collect();
    let legacy_q_bias_max = legacy_q_bias_slice.iter().map(|x| x.to_f32().abs()).fold(0.0f32, f32::max);
    eprintln!(
    ...
    use metallic::{
        foundry::{storage::Pooled, tensor::Tensor as FoundryTensor}, metals::rmsnorm::{RmsNorm as RmsNormKernel, RmsNormParamsResolved}, tensor::F16
    };

    let legacy_res1_tensor = FoundryTensor::<F16, Pooled>::new(
        &mut foundry,
        vec![d_model],
        metallic::tensor::TensorInit::CopyFrom(&legacy_out.residual_1),
    )?;
    let cross_output = FoundryTensor::<F16, Pooled>::new(&mut foundry, vec![d_model], metallic::tensor::TensorInit::Uninitialized)?;
    ...
    let cross_kernel = RmsNormKernel::new(
        &metallic::types::TensorArg::from_tensor(&legacy_res1_tensor),
        &metallic::types::TensorArg::from_tensor(&cross_output),
        &bindings.get("layer.ffn_norm_0")?,
        cross_params,
    );
    foundry.run(&cross_kernel)?;
    ...
    let legacy_ffn_norm_tensor = FoundryTensor::<F16, Pooled>::new(
        &mut foundry,
        vec![1, d_model],
        metallic::tensor::TensorInit::CopyFrom(&legacy_out.ffn_norm_out),
    )?;
    ...
    if let Some(legacy_gate) = &legacy_model.blocks[0].ffn_gate {
        ...
        eprintln!("  Legacy ffn_gate is None (using canonical?)");
...
fn test_dsl_vs_context_full_forward_parity() -> Result<(), MetalError> {
    ...
    let legacy_hidden_pre = legacy_forward_blocks_only(&legacy_model, &legacy_embedded, &mut ctx)?;
    ctx.synchronize();
    let legacy_hidden = legacy_model.forward(&legacy_embedded, &mut ctx)?;
    ctx.synchronize();
    eprintln!("Legacy hidden shape: {:?}", legacy_hidden.dims());

    let legacy_logits = legacy_model.output(&legacy_hidden, &mut ctx)?;
    ctx.synchronize();
    eprintln!("Legacy logits shape: {:?}", legacy_logits.dims());

    let legacy_logits_data: Vec<f16> = legacy_logits.as_slice().iter().map(|v| f16::from_f32(v.to_f32())).collect();
    ...
    let dsl_logits = bindings.get("logits")?;
    ...
    if max_diff > 0.5 {
        ...
        let dsl_hidden = bindings.get("hidden")?;
        let dsl_hidden_data = read_f16_buffer(&dsl_hidden);
        let legacy_hidden_data: Vec<f16> = legacy_hidden.as_slice().iter().map(|v| f16::from_f32(v.to_f32())).collect();
...
fn test_full_block_step_parity() -> Result<(), MetalError> {
    ...
    let legacy_input = legacy_model.embed(tokens, &mut ctx)?;
    ctx.synchronize();
    ...
    let legacy_out = legacy_layer0_outputs(&legacy_model, &legacy_input, &mut ctx)?;
