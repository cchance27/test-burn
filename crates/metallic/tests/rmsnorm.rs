...
fn cpu_rmsnorm(input: &[f32], feature_dim: usize, gamma: &[f32]) -> Vec<f32> {
    ...
    for row in 0..num_rows {
        ...
        for f in 0..feature_dim {
            let idx = row_start + f;
            output[idx] = (input[idx] / rms) * gamma[f];
        }
    }
...
fn run_f16_parity_test(cfg: TestConfig) {
    ...
    // Legacy RMSNorm
    let input_legacy = Tensor::<F16>::new(
        vec![cfg.num_rows, cfg.feature_dim],
        LegacyStorage::Pooled(&mut ctx),
        TensorInit::CopyFrom(&input_half),
    )
    .unwrap();
    let gamma_legacy = Tensor::<F16>::new(
        vec![cfg.feature_dim],
        LegacyStorage::Pooled(&mut ctx),
        TensorInit::CopyFrom(&gamma_half),
    )
    .unwrap();

    let y_legacy_tensor = ctx
        .call::<RMSNormOp>((input_legacy, gamma_legacy, cfg.feature_dim as u32), None)
        .unwrap();
    ...
    // New Foundry RMSNorm
    let input = FoundryTensor::<F16, Pooled>::new(&mut foundry, vec![total_elements], TensorInit::CopyFrom(&input_half)).unwrap();
    ...
    let kernel = RmsNorm::new(&input_arg, &output_arg, &gamma_arg, params);
...
fn quantize_rmsnorm_q8(input: &[f16], feature_dim: usize) -> (Vec<u8>, Vec<u8>) {
    let num_rows = input.len() / feature_dim;
    let blocks_per_row = (feature_dim + 7) / 8; // 8 weights per block for RMSNorm
...
fn run_q8_policy_test(cfg: TestConfig) {
    ...
    let result = foundry.run_with_policy::<PolicyQ8, _>(&kernel);

    // For now, just verify it compiles and runs - full correctness testing
    // would require updating the kernel's buffer layout
    println!("\n[Q8 Policy RMSNorm {}x{}]", cfg.num_rows, cfg.feature_dim);
    if let Err(ref e) = result {
        println!("  run_with_policy error: {:?}", e);
    }

    // The actual values won't match because we're not properly passing scale_bytes,
    // but this tests that the policy integration machinery works.
