...
use metallic::foundry::spec::{ModelSpec, TensorBindings};
...
fn test_e2e_forward_pass() {
    ...
    // Create foundry
    let mut foundry = metallic::foundry::Foundry::new().expect("Failed to create Foundry");
    ...
    assert!(bindings.len() > 0, "Should have some bindings");
