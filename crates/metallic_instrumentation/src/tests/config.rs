...
fn app_config_parses_rejects() {
    ...
    let _latency = ENABLE_PROFILING_VAR.set_guard(false).expect("latency flag should set");
    ...
    assert!(!resolved.enable_profiling);
    assert!(std::ptr::eq(
        resolved,
    ...
    assert!(!initialised.enable_profiling);
    ...
    let _latency = ENABLE_PROFILING_VAR.unset_guard();
    ...
    let _latency = EnvVarGuard::set(InstrumentEnvVar::EnableProfiling, "definitely");
    let _console = METRICS_CONSOLE_VAR.unset_guard();

    match AppConfig::from_env() {
        Err(AppConfigError::InvalidBoolean { name, value }) => {
            assert_eq!(name, InstrumentEnvVar::EnableProfiling.key());
