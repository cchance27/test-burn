...
- Metal: `src/metals/policies/policy_*.metal`
...
- Driver: `src/metals/matmul_gemv/simd_common.metal`
- Strategies (Rust): `src/metals/matmul_gemv/strategy.rs` (Implementation of `Canonical<Q>`)
- Policies (Metal):
  - `src/metals/policies/simd_gemv_f16_canonical.metal` (Generic over Quant)
  - `src/metals/policies/simd_gemv_q8_canonical.metal`
- Hooks (Rust): `src/metals/matmul_gemv/hooks.rs`
...
1. Create a new file:
   - `src/metals/policies/policy_q4.metal` (or whatever quant name you use).
...
1. A SIMD GEMV policy header:
   - `src/metals/policies/simd_gemv_q4_canonical.metal`
2. A hook in Rust:
