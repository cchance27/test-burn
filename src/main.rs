...
fn map_gpu_op_completed(op_name: &str) -> Option<Vec<String>> {
    ...
    if base_name.contains('/') && let Some(segments) = map_hierarchical_gpu_op(base_name) {
...
fn map_hierarchical_gpu_op(path: &str) -> Option<Vec<String>> {
    ...
    // Always start at Generation Loop -> Forward Step, then optionally Block N -> Stage -> Inner
