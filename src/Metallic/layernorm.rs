...
/// Ensure the layer normalization compute pipeline is compiled and cached on the Context.
pub fn ensure_layernorm_pipeline(ctx: &mut Context) -> Result<(), MetalError> {
    if ctx.layernorm_pipeline.is_some() {
        return Ok(());
    }
    ...
    kernel void layernorm_kernel(device float* input [[buffer(0)]],
        ...
        float var = (sum_sq / float(feature_dim)) - (mean * mean);
    ...
    let fn_name = NSString::from_str("layernorm_kernel");
    ...
    ctx.layernorm_pipeline = Some(pipeline);
    Ok(())
}

/// An operation that runs layer normalization over input tensors.
