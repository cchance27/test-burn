...
fn zeros_and_ones() {
    ...
    let t0 = Tensor::zeros(vec![2, 3, 4], &mut ctx).unwrap();
...
fn pool_growth_behavior() {
    ...
    let t1 = Tensor::zeros(large_dims.clone(), &mut ctx).unwrap();
...
fn pool_reset_invalidates_tensors() {
    ...
    let t1 = Tensor::zeros(vec![100, 100], &mut ctx).unwrap();
    ...
    let t3 = Tensor::zeros(vec![50, 50], &mut ctx).unwrap();
...
fn offset_correctness_across_chunks() {
    ...
    let t1 = Tensor::zeros(vec![1000, 1000], &mut ctx).unwrap(); // ~4MB
...
fn cpu_fill_with_offsets() {
    ...
    let _t1 = Tensor::zeros(vec![500, 500], &mut ctx).unwrap();
    ...
    // Test zeros as well
    let t3 = Tensor::zeros(vec![5, 5], &mut ctx).unwrap();
...
fn synchronization_contract() {
    ...
    let _t2 = Tensor::zeros(vec![10, 10], &mut ctx).unwrap();
...
fn strides_and_dtype() {
    ...
    let t = Tensor::zeros(vec![2, 3, 4], &mut ctx).unwrap();
