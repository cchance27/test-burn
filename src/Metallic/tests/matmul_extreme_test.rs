...
use objc2_metal::{MTLCommandBuffer as _, MTLCommandQueue};
...
fn cpu_matmul_scaled(
    ...
    for i in 0..m {
        for j in 0..n {
            ...
            let beta_c = if let Some(c_slice) = c {
                beta * c_slice[i * n + j]
            } else {
                0.0
            };
...
fn test_matmul_extreme_alpha_scaling() -> Result<(), MetalError> {
    ...
    let cpu_output = cpu_matmul_scaled(
        &a_data, m, k, &b_data, k, n, alpha, beta, None, false, false,
    );
    ...
    let a_desc_key = cache_keys::MpsMatrixDescriptorKey {
        rows: m,
        columns: k,
        row_bytes: k * bytes_per_elem,
    };
    let b_desc_key = cache_keys::MpsMatrixDescriptorKey {
        rows: k,
        columns: n,
        row_bytes: n * bytes_per_elem,
    };
    let result_desc_key = cache_keys::MpsMatrixDescriptorKey {
        rows: m,
        columns: n,
        row_bytes: n * bytes_per_elem,
    };
...
fn test_matmul_extreme_beta_accumulation() -> Result<(), MetalError> {
    ...
    let expected_result = cpu_matmul_scaled(
        &a_data,
        m,
        k,
        &b_data,
