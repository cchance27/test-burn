...
// CPU RoPE reference implementation for testing extreme values
fn cpu_rope_extreme(
    input: &[f32],
    batch: usize,
    seq_len: usize,
...
fn test_rope_extreme_large_position_values() -> Result<(), MetalError> {
    ...
    let cpu_output = cpu_rope_extreme(&input_data, batch, seq_len, dim, &cos_data, &sin_data);
