...
/// Ensure the elementwise subtract compute pipeline is compiled and cached on the Context.
pub fn ensure_sub_pipeline(ctx: &mut Context) -> Result<(), MetalError> {
    if ctx.sub_pipeline.is_some() {
        return Ok(());
    }
    ...
    let fn_name = NSString::from_str("sub_kernel");
    ...
    ctx.sub_pipeline = Some(pipeline);
    Ok(())
}

/// An operation that runs elementwise subtract over input tensors.
pub struct ElemwiseSub {
    pub a: Tensor,
    pub b: Tensor,
    pub out: Tensor,
    ...
    pub fn new(
        ...
        if a.dims() != b.dims() || a.dims() != out.dims() {
            return Err(MetalError::InvalidShape(format!(
                "ElemwiseSub: input shapes must match, got a={:?}, b={:?}, out={:?}",
                a.dims(),
                b.dims(),
                out.dims()
            )));
