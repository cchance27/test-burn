...
/// Ensure the elementwise divide compute pipeline is compiled and cached on the Context.
pub fn ensure_div_pipeline(ctx: &mut Context) -> Result<(), MetalError> {
    if ctx.div_pipeline.is_some() {
        return Ok(());
    }
    ...
    let fn_name = NSString::from_str("div_kernel");
    ...
    ctx.div_pipeline = Some(pipeline);
    Ok(())
}

/// An operation that runs elementwise divide over input tensors.
pub struct ElemwiseDiv {
    pub a: Tensor,
    pub b: Tensor,
    pub out: Tensor,
    ...
    pub fn new(
        ...
        if a.dims() != b.dims() || a.dims() != out.dims() {
            return Err(MetalError::InvalidShape(format!(
                "ElemwiseDiv: input shapes must match, got a={:?}, b={:?}, out={:?}",
                a.dims(),
                b.dims(),
                out.dims()
            )));
...
impl Operation for ElemwiseDiv {
    fn encode(
        &self,
