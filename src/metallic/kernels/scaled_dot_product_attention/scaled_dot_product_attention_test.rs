...
fn test_scaled_dot_product_attention_kernel() -> Result<(), MetalError> {
    ...
    let result_tensor = ctx.call::<ScaledDotProductAttentionOptimizedOp>((&q, &k, &v, false, 0, 1))?;
    ctx.synchronize();
...
fn test_scaled_dot_product_attention_kernel_causal() -> Result<(), MetalError> {
    ...
    let result_tensor = ctx.call::<ScaledDotProductAttentionOptimizedOp>((&q, &k, &v, true, 0, 1))?;
    ctx.synchronize();
